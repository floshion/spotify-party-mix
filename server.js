...
// ========== [ADDED] Helpers: audio-features + compat ==========
...
function mapKeyModeToText(key, mode) {
  const names = ['C','C#','D','D#','E','F','F#','G','G#','A','A#','B'];
  if (key == null || key < 0 || key > 11) return null;
  return names[key] + (mode === 1 ? '' : 'm');
}

function compatibilityDistance(a, b) {
  // a,b: { tempo, key } ; Δ BPM (avec 2x/0.5x) + proximité de clé
  let d = 0;
  if (Number.isFinite(a.tempo) && Number.isFinite(b.tempo)) {
  ...
  if (a.key && b.key) d += (a.key === b.key ? 0 : 3);
  else d += 5;
  return d;
}
...
function reasonText(target, cand) {
  const parts = [];
  if (Number.isFinite(target.tempo) && Number.isFinite(cand.tempo)) {
    const tempos = [cand.tempo, cand.tempo*2, cand.tempo/2];
    const dt = Math.min(...tempos.map(x => Math.abs(target.tempo - x)));
    parts.push(`tempo ~${Math.round(target.tempo)} (Δ≈${Math.round(dt)})`);
  }
...
// ========== [ADDED] Page de test
app.get('/test-recos', (_req, res) => {
  res.sendFile(path.join(__dirname, 'static', 'test-recos.html'));
});

// ========== [ADDED] Features du morceau en cours
app.get('/ext/current-features', async (_req, res) => {
  try {
    const nowRes = await fetch('https://api.spotify.com/v1/me/player/currently-playing', {
      headers: { Authorization: 'Bearer ' + access_token }
    });
    if (!nowRes.ok) return res.json({ playing: false });
    const nowJson = await nowRes.json();
    if (!nowJson || !nowJson.item) return res.json({ playing: false });

    const cur = {
      id:        nowJson.item.id,
      name:      nowJson.item.name,
      artists:   nowJson.item.artists?.map(a => a.name).join(', ') || '',
      duration_ms: nowJson.item.duration_ms,
      uri:       nowJson.item.uri,
      album:     nowJson.item.album?.name || ''
    };

    const f = await spotifyAudioFeatures(access_token, cur.id);
    const features = f ? { tempo: f.tempo ?? null, key: mapKeyModeToText(f.key, f.mode), mode: f.mode } : null;

    res.json({ playing: true, track: cur, features });
  } catch (e) {
    console.error(e);
...
// ========== [ADDED] Suggestions compatibles (3–4) via Last.fm + filtrage
app.get('/ext/suggest', async (req, res) => {
  try {
    const LASTFM_API_KEY = process.env.LASTFM_API_KEY;
    if (!LASTFM_API_KEY) return res.status(400).json({ error: 'LASTFM_API_KEY missing' });

    // 1) Current + features cible
    const nowRes = await fetch('https://api.spotify.com/v1/me/player/currently-playing', {
      headers: { Authorization: 'Bearer ' + access_token }
    });
    if (!nowRes.ok) return res.json([]);
    const nowJson = await nowRes.json();
    if (!nowJson || !nowJson.item) return res.json([]);
    const current = {
      id: nowJson.item.id,
      name: nowJson.item.name,
      artists: nowJson.item.artists?.map(a => a.name).join(', ') || ''
    };
    const cf = await spotifyAudioFeatures(access_token, current.id);
    const target = cf ? { tempo: cf.tempo ?? null, key: mapKeyModeToText(cf.key, cf.mode) } : {};

    // 2) Last.fm similaires
    const name = req.query.name || current.name;
    const artists = (req.query.artists || current.artists || '').split(',')[0].trim();
    ...
    const lr = await fetch('https://ws.audioscrobbler.com/2.0/?' + lf.toString());
    const lj = lr.ok ? await lr.json() : {};
    const sims = lj?.similartracks?.track || [];

    // 3) Pour chaque candidat → cherche 1 track Spotify + récup features + score
    const results = [];
    for (const t of sims) {
      ...
      const qs = new URLSearchParams({ q: `track:${tName} artist:${tArtist}`, type: 'track', limit: '1' });
      const sr = await fetch('https://api.spotify.com/v1/search?' + qs.toString(), {
        headers: { Authorization: 'Bearer ' + access_token }
      });
      if (!sr.ok) continue;
      const sj = await sr.json();
      const tr = sj?.tracks?.items?.[0];
      if (!tr) continue;

      const f = await spotifyAudioFeatures(access_token, tr.id);
      if (!f) continue;
      const cand = { tempo: f.tempo ?? null, key: mapKeyModeToText(f.key, f.mode) };
      const dist = compatibilityDistance(target, cand);
      ...
      results.push({
        ...
        reason: reasonText(target, cand),
        dist
      });
    }

    results.sort((a,b) => a.dist - b.dist);
    const limit = Math.min(parseInt(req.query.limit || '4', 10) || 4, 4);
