...
setInterval(async () => {
  try {
    ...
    // 204 = pas de device actif -> on ne parse pas, on sort
    if (resp.status === 204) return;

    // Quelques API Spotify renvoient un body vide même en 200
    const txt = await resp.text();
    if (!txt) return;

    let s;
    try { s = JSON.parse(txt); } catch { return; }

    if (!s) return;

    const inOurPlaylist =
      s.context && s.context.uri === ('spotify:playlist:' + SOURCE_PLAYLIST);

    const stuck =
      (s.is_playing === false) &&
      (s.progress_ms === 0) &&
      (s.currently_playing_type === 'track');

    if (stuck && inOurPlaylist) {
      const n = await guardedFetch(
      ...
      if (!n.ok) {
        await guardedFetch('https://api.spotify.com/v1/me/player/play', {
          ...
          body: JSON.stringify({
            context_uri: 'spotify:playlist:' + SOURCE_PLAYLIST,
            position_ms: 0
          })
        });
      }
    ...
    // On garde un log discret pour ne pas spammer
    console.warn('watchdog soft error', e?.message || e);
  }
}, 7000);
